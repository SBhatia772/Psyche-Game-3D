...
public class VattalusSpaceshipController : MonoBehaviour
{
    ...
    public List<GameObject> landingGearColliders = new List<GameObject>();
    ...
    private void FixedUpdate()
    {
        ...
        if (levelController.GetComponent<LevelController>().gameOver == false)
        {
            ...
            {
                Debug.Log("G Pressed");
                if (landingGearColliders[0].GetComponent<BoxCollider>().enabled)
                {
                    Debug.Log("Disabling Collider");
                    foreach (var landingGear in landingGearColliders)
                    {
